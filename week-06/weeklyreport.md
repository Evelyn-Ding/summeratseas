...
- read yu-wen's paper
- low level features
- high level concepts -> goes one step BEYOND the concept bottleneck FW paper by prompting ALM (flexibility) - U still have to decide which concepts matter, but the flexibility comes in when u can simply change the prompt to chnage the concepts, the underlying architecture (of ALM) doesn't need to be changed
## Wednesday, 7/15
