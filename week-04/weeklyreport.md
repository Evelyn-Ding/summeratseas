...
- I compiled the csvs into the [torgo egemaps summary csv](https://github.com/Evelyn-Ding/summeratseas/blob/main/week-04/torgo_egemaps_summary.csv). The csv has around 8,000 rows (representing audio files) and 88 columns (representing eGeMAPS features).
...
- Step (2) was to see if the output matched my expectations. I wanted to make sure the csv had the correct dimensions, which it did. I also looked at the data distribution - slightly imbalanced by a 2:1 ratio, with more control than dysarthric labels. I confirmed that there were 0 missing values.
...
- We finalized the Data Transfer and Use Agreement for the Bridge2AI-Voice dataset and sent it off to the Columbia contracts team for signatures! Once we have all the signatures, Dr. Hirschberg will submit our forms to request access to the dataset through Synapse.
