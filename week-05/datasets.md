...
## Appendix: MDVP Feature Glossary

MDVP stands for **Multi-Dimensional Voice Program** — a standardized clinical software by **Kay Elemetrics** (later rebranded as KayPENTAX/PENTAX Medical) that extracts acoustic measures of voice quality from sustained vowel phonation. MDVP runs on their **Computerized Speech Lab (CSL)** hardware/software platform, which also uses the proprietary `.nsp` audio format — which is exactly why the Saarbrücken Voice Database (SVD) stores recordings as `.nsp` files: it was recorded using the same Kay Elemetrics/CSL clinical setup. "MDVP-style" means features computed using the same definitions, even if not produced by the original software.

| Feature | Full Name | What it measures |
|---------|-----------|-----------------|
| Jitter | Jitter (local) | Cycle-to-cycle variation in pitch period (F0 irregularity) |
| RAP | Relative Average Perturbation | Smoothed short-term jitter over 3-cycle window |
| PPQ | Pitch Perturbation Quotient | Smoothed short-term jitter over 5-cycle window |
| Shimmer | Shimmer (local, dB) | Cycle-to-cycle variation in amplitude (loudness irregularity) |
| APQ | Amplitude Perturbation Quotient | Smoothed short-term shimmer over 11-cycle window |
| NNE | Normalized Noise Energy | Ratio of noise to harmonic energy in the voice signal |
| HNR | Harmonics-to-Noise Ratio | How much of the signal is periodic (harmonic) vs. noise |
| GNE | Glottal-to-Noise Excitation ratio | Whether voice is excited by glottal pulses or turbulent noise |
